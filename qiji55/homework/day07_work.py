...
# 使用reduce函数来统计列表li中元素转换为字符串后包含字符1的元素个数。
# reduce函数的参数：
# 第一个参数是一个匿名函数lambda x, y: x+(1 if '1' in str(y) else 0)，
# 这个函数接受两个参数x和y。
# 在每次迭代中，x是累积的结果（初始值为 0，由reduce的第三个参数指定），
# y是列表中的下一个元素。
# 如果y转换为字符串后包含1，就将x加 1，否则x不变。
# 第二个参数是要处理的列表li。
# 第三个参数是初始值 0，表示累积结果的初始状态。
from functools import reduce

count_ones = reduce(lambda x, y: x + (1 if '1' in str(y) else 0), li, 0)

print("列表中包含数字1的次数为:%d" % count_ones)
